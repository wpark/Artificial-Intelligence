...
def enhancedFeatureExtractorDigit(datum):
    ...
    1. Checks if the left region has more pixels that are not blank than the right region
    2. Checks if the top region has more pixels that are not blank than the bottom region
    3. Checks if each row has at least more than five pixels that are not blank
    4. Checks if each column has at least more than five pixels that are not blank
    ##
    """
    # q4
    ...
    def leftRegion(datum, features):
        # Sets the feature to be True, if more pixels occupied in the left region
        #  are occupied than the right region
        left, right = 0, 0
        for x in range(width):
            for y in range(height):
                if (datum.getPixel(x, y) == True):
                    if x < int(width/2):
                        left += 1
                    else:
                        right += 1
        ...
        if left >= right:
            features['leftRegion'] = True
        else:
            features['leftRegion'] = False


    def topRegion(datum, features):
        # Sets the feature to be True, if more pixels occupied in the top region
        #  are occupied than the bottom region
        top, bottom =  0, 0
        for x in range(width):
            for y in range(height):
                if (datum.getPixel(x, y) == True):
        ...
        if top >= bottom:
            features['topRegion'] = True
        else:
            features['topRegion'] = False


    def rowPixels(datum, features):
        # Sets the feature for each row to be True if at least more than 5 pixels in the row 
        #  are not blank
        rows = util.Counter()
        for x in range(width):
            for y in range(height):
                #print datum.getPixel(x,y), datum.getPixel(x,y) == True
                if datum.getPixel(x, y) > 0:
                    rows[x] += 1
        ...
        for row in rows:
            ...
            features['row_'+str(row)] = (rows[row] > 5)
              

    def colPixels(datum, features):
        # Sets the feature for each row to be True if at least more than 5 pixels in the column 
        #  are not blank
        columns = util.Counter()
        for x in range(width):
            for y in range(height):
                #print datum.getPixel(x,y), datum.getPixel(x,y) == True
                if datum.getPixel(x, y) > 0:
        ...
        for col in columns:
            features['column_'+str(col)] = (columns[col] > 5)


    # calling the feature methods defined above
    leftRegion(datum, features)
    topRegion(datum, features)
    rowPixels(datum, features)
    colPixels(datum, features)
...
def enhancedPacmanFeatures(state, action):
    ...
    def gridToList(grid):
        # Helper function that returns a list of positions that were True in grid
        gridList =[]
    ...
    def foodFeature(state):
        foodList = gridToList(state.getFood())
        minDist = 10000000

        for food in foodList:
            dist = util.manhattanDistance(state.getPacmanPosition(), food)
            if minDist > dist:
                minDist = dist
                #print minDist
        features['foodDist'] = -((10/minDist)^2)

    def ghostFeature(state):
        minDist = 10000000

        for ghost in state.getGhostPositions():
            dist = util.manhattanDistance(state.getPacmanPosition(), ghost)
            if dist == 0:
        ...
        features['ghostDist'] = 3/minDist

    # calling feature methods defined above
    foodFeature(nextState)
    ghostFeature(nextState)
